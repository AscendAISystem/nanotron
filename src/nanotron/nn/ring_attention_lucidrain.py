...
class RingFlashAttentionCUDAFunction(Function):
    ...
    def forward(
        ...
        if is_npu_available():
            raise RuntimeError(
                "RingFlashAttentionCUDAFunction requires Triton and flash_attn, "
                "which are not available on NPU. Use 'sdpa' or 'flash_attention_2' instead."
            )
        if 'triton' not in sys.modules:
            raise RuntimeError(
                "RingFlashAttentionCUDAFunction requires Triton, "
...
# triton flash attention (guarded for NPU compatibility)
_HAS_TRITON = False
try:
