...
def ring_flash_attn_varlen_forward(
    ...
    if is_npu_available():
        raise RuntimeError(
            "ring_flash_attn_varlen_forward requires Triton and flash_attn, "
            "which are not available on NPU. Use 'sdpa' or 'flash_attention_2' instead."
        )
    if 'triton' not in sys.modules:
        raise RuntimeError(
            "ring_flash_attn_varlen_forward requires Triton, "
            "which is not installed. Install it with: pip install triton"
        )
...
def ring_flash_attn_varlen_backward(
    ...
    if is_npu_available():
        raise RuntimeError(
            "ring_flash_attn_varlen_backward requires Triton and flash_attn, "
            "which are not available on NPU. Use 'sdpa' or 'flash_attention_2' instead."
        )
    if 'triton' not in sys.modules:
        raise RuntimeError(
            "ring_flash_attn_varlen_backward requires Triton, "
            "which is not installed. Install it with: pip install triton"
        )
...
## triton_utils.py — guarded import to prevent crash on NPU
_HAS_TRITON = False
try:
    import triton
