...
class StarcoderRotaryEmbedding(nn.Module):
    ...
    def init_rotary_embeddings(self):
        ...
        assert self.inv_freq.device.type == get_default_device().type
        ...
        self.inv_freq = 1.0 / (
            self.base ** (torch.arange(0, self.head_dim, 2, dtype=torch.float, device=get_default_device()) / self.head_dim)
        )
